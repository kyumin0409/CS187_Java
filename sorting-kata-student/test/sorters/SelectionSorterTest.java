...
public class SelectionSorterTest {
	...
	public void before() {
		...
		emptySorter = new SelectionSorter<Integer>(emptyList,
				INTEGER_COMPARATOR);

		sortedList = new ArrayBasedSwapList<Integer>(new Integer[] { -3, -1, 0,
				2, 4 });
		sortedSorter = new SelectionSorter<Integer>(sortedList,
				INTEGER_COMPARATOR);
		...
		reversedSorter = new SelectionSorter<String>(reversedList,
				STRING_COMPARATOR);

		firstAsLastList = new ArrayBasedSwapList<Integer>(new Integer[] { 5, 6,
				7, 8, 9, 10, 4 });
		firstAsLastSorter = new SelectionSorter<Integer>(firstAsLastList,
				INTEGER_COMPARATOR);
	}
	...
	public void testRandomLists() {
		...
		for (SwapList<Integer> list : randomizedLists) {
			AbstractSorter<Integer> sorter = new SelectionSorter<Integer>(list,
					INTEGER_COMPARATOR);
